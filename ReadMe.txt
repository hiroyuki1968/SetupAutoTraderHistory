========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : SetupAutoTraderHistory
========================================================================


AppWizard が生成した SetupAutoTraderHistory には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには SetupAutoTraderHistory を構成している各ファイルの概要説明が含まれています。

SetupAutoTraderHistory.dsp
    このファイル (プロジェクト ファイル) はプロジェクト レベルの情報を含み、
    シングル プロジェクトまたはサブ プロジェクトのビルドに使用されます。他の
    ユーザーとプロジェクト ファイル (.dsp) を共有できますが、メイク ファイル
    はローカルにエクスポートしてください。

SetupAutoTraderHistory.h
    このファイルはアプリケーションの中心となるインクルードファイルです。この
    ファイルは他のプロジェクトの固有のインクルードファイル（Resource.h  も含
    みます） をインクルードし、また CSetupAutoTraderHistoryApp アプリケーションクラスを宣
    言します。

SetupAutoTraderHistory.cpp
    このファイルは CSetupAutoTraderHistoryApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。

SetupAutoTraderHistory.rc
...
res\SetupAutoTraderHistory.ico
    このファイルは、アプリケーションのアイコンとして使用されるアイコンファイ
    ルです。このアイコンはリソース ファイル SetupAutoTraderHistory.rc によってインクルード
    されます。

res\SetupAutoTraderHistory.rc2
...
SetupAutoTraderHistoryDlg.h, SetupAutoTraderHistoryDlg.cpp - ダイアログ
    これらのファイルは CSetupAutoTraderHistoryDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な SetupAutoTraderHistory.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) SetupAutoTraderHistory.pch 
